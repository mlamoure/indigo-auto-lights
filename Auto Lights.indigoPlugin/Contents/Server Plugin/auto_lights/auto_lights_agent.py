...
def auto_lights_agent(config: AutoLightsConfig, orig_zones: List[Zone]) -> int:
    """
    Main automation function that processes a series of lighting zones.

    Args:
        config (AutoLightsConfig): The configuration object that governs behavior.
    ...
    if config.debug:
        ...
        debug_str = (
            ...
            + str(config.get_timed_brightness())
            + "\n"
        )
        debug_str = (
            debug_str
            + "       config.isDayTimeHours(): "
            + str(config.is_day_time_hours())
            + "\n"
        )
        debug_str = (
            debug_str + "       config.guestMode: " + str(config.guest_mode) + "\n"
        )
    ...
    for zone in zones:
        ...
        if zone.lighting_periods is None:
            ...
            if not config.someone_home or config.gone_to_bed:
                if config.debug:
                    indigo.server.log(
                        "auto_lights script DEBUG for Zone '"
                        + zone.name
        ...
        if not config.someone_home:
        ...
        elif zone.turn_off_while_sleeping and config.gone_to_bed:
            zone.target_brightness = 0
            ...
            if (
                ...
                if config.is_day_time_hours():
                    ...
                    zone.target_brightness = config.get_timed_brightness()
        ...
        if (
            ...
            and config.someone_home
            and not config.gone_to_bed
        ):
    ...
    if config.debug:
        for zone in zones:
            debug_str = (
                debug_str
                + zone.write_debug_output(config)
                + "\n----------------------------------------\n"
            )
