...
If a light stops responding reliably, Auto Lights now tries to fail safely instead of hammering your network:

- **Device failure suppression**  
  If a light fails to confirm **3 commands in a row**, Auto Lights temporarily stops sending commands to that specific device. Other lights in the zone can continue working normally.

- **Brightness is confirmed within 1 percentage point**  
  Some device plugins carry brightness in a native range (zigbee 0–254, Z-Wave 0–99) and report a percentage back one point below the one that was commanded — ask for 30%, the device honestly says 29%. Auto Lights accepts that, so healthy dimmers are not suppressed for a rounding difference. Fully off and fully on must still match exactly, so a light that failed to switch is still treated as a failure.

- **How the device starts working again**  
  Suppression clears automatically once the device's actual state matches what Auto Lights expects. In practice, that usually means the device starts reporting correct on/off or brightness state again after the hardware problem is fixed.

- **Why a zone may appear to lock itself**  
  A flaky device that never reaches its target can look like a manual override. When that happens, the zone may create a normal lock to avoid fighting the state change. Once the lock expires, Auto Lights re-evaluates the zone and resumes normal automation.

- **What the warning logs mean**
  - `⚠️ Device 'X' failed to confirm 3 consecutive commands — suppressing until it responds`
  - `⚠️ Zone 'Y': re-evaluation rate limit reached — deferring further re-evaluation. Check for unresponsive devices.`

- **What you should do**
  - Repair or replace the unresponsive device
  - Check Indigo's protocol logs, especially Z-Wave logs, for repeated transport errors
  - Restart the Auto Lights plugin if you want to clear in-memory suppression state immediately
  - Re-test the zone once the device is healthy to confirm that normal automation resumes
...
3. **Adjust Dimmer Brightness**: If turned on, the brightness of a dimmer device will be set to:
    - [1 - (current luminance / darkness threshold)] x 100

   The darkness threshold is Minimum Luminance; while Luminance Hysteresis is holding the Zone dark it is Minimum
   Luminance + Luminance Hysteresis, so the computed level stays above zero and decays smoothly across the band.
...
- **Luminance Hysteresis**: Extra luminance required before the Zone stops being considered dark, in the same units as
  Minimum Luminance (i.e. whatever your luminance sensors report). Defaults to `0`, which disables the band — darkness is
  then the same bare comparison as before.

#### Why hysteresis matters

With no hysteresis, darkness is a bare `average luminance < minimum luminance` test. A reading that hovers near the
threshold therefore turns the lights on and off repeatedly — on a broken-cloud afternoon this can cycle every couple of
minutes for as long as someone is in the room.

This is worse than ordinary sensor noise would suggest whenever **the luminance sensor is in the room it controls**,
because it also measures the lights it is gating. Turning them on raises its own reading, which can push it back over
the threshold, which turns them off, which lowers the reading again. That is a feedback loop, not just jitter.

Hysteresis is applied **only when leaving the dark state**, so it never delays turning lights on — only turning them
off. With a Minimum Luminance of `2500` and a hysteresis of `500`, lights come on below `2500` and do not go off until
the Zone reaches `3000`.

When **Adjust Dimmer Brightness** is on, the brightness calculation divides by this same widened threshold, so inside
the band the level tapers toward 1% rather than snapping to off; the lights only turn off once the Zone leaves the
band.

> **NOTE:** Set the value wider than your lights' own contribution at the sensor, or the loop will still ring. Measure
> it by reading the sensor with the Zone's lights off and then at full brightness, after dark, and take the difference.

The band is cleared when the plugin restarts, so a Zone that was being held dark inside the band re-evaluates against the
plain Minimum Luminance on the first reading after a reload. On a borderline afternoon that can turn the lights off once,
immediately after a restart.

#### When luminance sensors cannot be read

This applies whether or not you use hysteresis.

If a Zone has luminance devices configured but none of them can be read — deleted device, flat battery, source plugin
stopped, a sensor reporting a non-numeric value while its plugin starts up — Auto Lights **holds the previous darkness
decision** rather than assuming darkness, and logs a warning naming the devices. Assuming darkness would be
indistinguishable from a real reading and would silently switch the Zone's lights on.

- The warning is logged **once per outage**, not once per evaluation, and a matching message is logged when readings
  recover.
- If some sensors are readable and others are not, the readable ones are averaged and the thinning is logged.
- After five minutes with no real reading the warning **escalates to an error** on every evaluation. Holding is meant to
  bridge a dropped report, not to run a Zone blind all day.

A Zone with **no** luminance devices configured is a different thing entirely — that is a deliberate choice to run
without a luminance gate, is always treated as dark, and logs nothing.
...
- **Limit Brightness**: Cap the maximum brightness if you don’t want lights going above, say, 50% at night. This applies
  to every brightness Auto Lights calculates for the period, whether it came from Adjust Dimmer Brightness, a per-device
...
By default every light in a Zone is turned on to the same level. The **Device to Lighting Period Mappings** matrix in
the Zone configuration lets you override that per light, per period.

Each cell in the matrix offers three kinds of answer:

- **Include in Period** — the light participates and takes the Zone's normal brightness. This is the default.
- **Exclude from Period** — Auto Lights will not turn this light on during the period. It is still turned off when the
  Zone becomes unoccupied.
- **A brightness (1–100)** — the light participates and is driven to exactly this level for this period.

This is what you want when the fixtures in a room shouldn't all sit at the same level. A kitchen might run its task
spots at 10% late in the evening while the pendants stay at 60% and the cabinet strip sits at 40% — three levels, one
Zone, one period.

Notes:

- A per-device level takes precedence over **Adjust Dimmer Brightness**. If you set both, the explicit level wins for
  that light, and the luminance calculation still applies to every other light in the Zone.
- The period's **Limit Brightness** is still a ceiling. A light set to 80 in a period capped at 30 is driven to 30.
- Levels are 1–100. Use *Exclude from Period* rather than 0 to keep a light off — 0 is not a valid level, and a cell
  holding one is ignored with a warning in the Indigo log.
- Leaving every cell on *Include in Period* reproduces the behaviour of earlier versions exactly, so existing
