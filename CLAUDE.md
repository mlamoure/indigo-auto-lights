...
**Hysteresis and Adjust Brightness**: `Zone.calculate_target_brightness()`'s `adjust_brightness` formula divides by `effective_darkness_threshold`, not the bare `minimum_luminance`. Inside the hysteresis band the zone is held dark against the widened threshold (`minimum_luminance + luminance_hysteresis`), and dividing by the bare minimum there made the term go negative — clamping to 0 and switching dimmers off in the branch that was supposed to be turning them on (issue #7). Dividing by the effective threshold instead guarantees a computed level ≥ 1 while the zone is genuinely dark; the `max(0, …)` clamp remains only as defence-in-depth against the threshold moving between `is_dark()`'s read and the formula's (variable-backed thresholds re-read Indigo on every access).

**Configuration Schema**: JSON-based configuration with schema validation for both global settings and individual zones.

**Brightness Confirmation Tolerance**: `utils._brightness_matches()` confirms an intermediate brightness target within `BRIGHTNESS_CONFIRM_TOLERANCE` (1 percentage point); 0 and 100 stay exact. Device plugins that scale a percentage through a native range (zigbee 0–254, Z-Wave 0–99) truncate in both directions, so 95 of the 101 percentages never round-trip exactly and exact-equality confirmation suppressed healthy dimmers for the life of the lighting period. Every comparison of actual-vs-target brightness in the plugin runs through `_check_confirm()`/`is_device_at_target()`, so the send path, the `has_brightness_changes()` lock check and the suppression auto-recovery gate cannot diverge.

**Device Failure Suppression**: Prevents infinite command loops when devices fail to confirm state changes (e.g., unreachable HA Agent devices, or flaky Z-Wave nodes whose brightness state never updates in Indigo). Per-device consecutive failure tracking in `zone._device_fail_count` suppresses a device after `MAX_CONSECUTIVE_FAILURES` (3) unconfirmed commands. `send_to_indigo()` sends once and polls for up to 2 seconds for confirmation (Indigo handles protocol-level retries), returning `bool` (True=confirmed, False=settle timeout). Suppressed devices are skipped in `has_brightness_changes()` and `save_brightness_changes()`, breaking the re-evaluation loop. **Auto-recovery is gated**: `process_device_change()` only clears the failure count when the device's actual state matches the zone's current `target_brightness` for that device (verified via `utils.is_device_at_target`). A bare `deviceUpdated` callback is not enough — for a flaky device, Indigo emits state diffs (lastChanged, partial reports, polling responses) that look like activity but mean nothing, and the old "clear on any update" logic let suppression be defeated indefinitely.

**Re-evaluation Rate Limit**: Belt-and-suspenders cap on the writer-thread re-evaluation path (`save_brightness_changes` → `_writer` → `process_zone`). Each zone tracks a sliding window of recent writer-initiated re-evals via `Zone._can_reeval()`; once `MAX_REEVAL_BURST` (5) re-evals have occurred within `REEVAL_WINDOW_SECONDS` (30), further writer-initiated re-eval is deferred until the window drains, with a one-time WARNING logged. External triggers (sensor changes, manual user actions) bypass this gate entirely and never touch the deque, so legitimate activity is unaffected. This is a defense-in-depth guard that should never fire in normal operation — if it does, something is wrong with a device.

**Lock + Suppression Interaction**: A flaky device that never reaches its target can have its persistent state-vs-target mismatch interpreted as a manual user override by the lock-creation path in `process_device_change`, triggering a zone lock. While locked, `process_zone` short-circuits and no writes happen at all. When the lock expires, the zone re-evaluates: if the broken device is still suppressed, `has_brightness_changes()` skips it and the *other* devices in the zone resume normal automation. So a self-induced lock is actually a graceful failure mode — it pauses the zone briefly, suppression engages in the meantime, and the rest of the zone keeps working. Suppression state persists across lock/unlock cycles; the broken device only rejoins automation when its actual state finally matches its target.
